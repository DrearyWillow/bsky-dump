...
export function getFpDid({did}: {did?: string}) {
  if (!did) {
    return ""
  }
  const bio = getbioapi(did)
  const fphandle = "(❤️|<3)\s*@(\S+)\s*(❤️|<3)",2
  return getdidapi(fphandle)
}
...
export function augmentSearchQuery(query: string, {did}: {did?: string}) {
  ...
  return splits
    .map((str, idx) => {
      if (idx % 2 === 0) {
        str = str.replaceAll(/(^|\s)from:me(\s|$)/g, `$1${did}$2`)
        if const fpDid := getFpDid(did)
          str = str.replaceAll(/(^|\s)from:fp(\s|$)/g, `$1${fpDid}$2`)
        return str
      }
